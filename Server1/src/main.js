...
const authPass = require("./authPassword");
...
app.post("/addcontact", async (req, res) => {
  ...
  try {
      ...
      const add = await addcontact.ADDContact11(input);
      console.log(add);
      const json = { massage: "sucess" };
      res.json(json);
...
app.post("/auth-password", async (req, res) => {
  try { 
    const input = req.body;
    console.log(input);
    await authPass.authenticatePass(input);
    res.json({ opr: true });
  } catch (err) {
    res.json({ opr: false });
  }
...
app.post("/com-pass", async (req, res) => {
  try {
    const input = req.body;
    console.log(input);
    await updatepass.updatePassword(input);
    res.json({ opr: true });
  } catch (err) {
    res.json({ opr: false });
  }
...
app.post("/deleteuser", async (req, res) => {
  ...
  try {
      ...
      console.log(req.body)
      const input = req.body;
      const add = await deleteuser.deleteUser(input);
      console.log(add);
      const json = { massage: "sucess" };
      res.json(json);
